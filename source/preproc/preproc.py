...
# Variables
DEFAULT_TECHNICAL_INDICATOR_LIST = ["macd", "rsi", "cci", "adx"]
...
def _from_adjusted_to_ohlcv(df):
    ...
    x["ajexdi"] = x["ajexdi"].apply(lambda index: 1 if index == 0 else index)
...
class Preprocessor(DatasetIOStaticMethod):
    def __init__(
        ...
        # Declare parameters
        if (
            isinstance(technical_indicator_list, list)
            and technical_indicator_list
        ):
            self.technical_indicator_list = technical_indicator_list
        else:
            self.technical_indicator_list = DEFAULT_TECHNICAL_INDICATOR_LIST
        if isinstance(user_defined_features, dict) and user_defined_features:
